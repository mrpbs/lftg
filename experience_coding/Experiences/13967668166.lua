-- Ultimate Safe Welcome Screen Sniper
task.spawn(function()
    local targetContainer = (get_hidden_gui and get_hidden_gui()) or (gethui and gethui()) or game:GetService("CoreGui")
    
    local function huntWelcomeScreen(child)
        -- Wait a split second to let the UI elements load inside the ScreenGui
        task.wait(0.1)
        
        -- Step 1: Whitelist the good menus so they NEVER get deleted
        local safeNames = {
            ["flamesadmingui"] = true,
            ["minigamemenugui"] = true,
            ["notification library"] = true,
            ["invisibleui"] = true,
            ["robloxgui"] = true,
            ["outfitmanagerui"] = true
        }
        if safeNames[string.lower(child.Name)] then return end

        -- Step 2: Hunt for the Welcome text in any other random ScreenGui
        if child:IsA("ScreenGui") then
            for _, desc in ipairs(child:GetDescendants()) do
                if desc:IsA("TextLabel") and string.find(string.lower(desc.Text), "welcome") then
                    child.Enabled = false
                    child:Destroy()
                    break
                end
    ...
    -- Listen for new GUIs being added
    local conn = targetContainer.ChildAdded:Connect(huntWelcomeScreen)

    -- Stop hunting after 15 seconds to save performance
    task.wait(15)
    conn:Disconnect()
end)
