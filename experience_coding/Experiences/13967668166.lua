-- Flames Hub UI Tracker & Debugger
task.spawn(function()
    local CoreGui = game:GetService("CoreGui")
    local hiddenGui = (get_hidden_gui and get_hidden_gui()) or (gethui and gethui()) or CoreGui
    
    warn("🛠️ UI Debugger Started! Watching for Welcome Screen...")

    local function logUI(child)
        -- Log any new main GUI windows
        if child:IsA("ScreenGui") then
            warn("🔍 [DEBUG] ScreenGui Loaded: '" .. child.Name .. "' | Hiding in: " .. tostring(child.Parent))
        
        -- Log any text labels, specifically looking for "welcome"
        elseif child:IsA("TextLabel") or child:IsA("TextBox") then
            local text = string.lower(child.Text)
            if string.find(text, "welcome") or string.find(text, "flames hub") then
                warn("🚨 [DEBUG] Suspect Text Found: '" .. child.Text .. "'")
                
                -- Find exactly which ScreenGui this text belongs to
                local screenGui = child:FindFirstAncestorWhichIsA("ScreenGui")
                if screenGui then
                    warn("   ↳ This text belongs to ScreenGui: '" .. screenGui.Name .. "'")
                else
                    warn("   ↳ Could not find parent ScreenGui for this text.")
                end
            end
        end
    end

    -- Listen to both normal CoreGui and Executor Hidden GUIs
    local conn1 = CoreGui.DescendantAdded:Connect(logUI)
    local conn2 = hiddenGui.DescendantAdded:Connect(logUI)
    
    -- Stop tracking after 20 seconds so it doesn't lag your game
    task.wait(20)
    conn1:Disconnect()
    if conn2 then conn2:Disconnect() end
    warn("🛑 UI Debugger Stopped tracking.")
end)
