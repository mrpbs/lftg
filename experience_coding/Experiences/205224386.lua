...
local function executor_contains(substr)
    if typeof(executor_string) ~= "string" then return false end
...
getgenv().get_char = getgenv().get_char or function(Player)
    ...
    local function hookchar(char)
        ...
        if hum and hum:IsDescendantOf(game) then diedconn = hum.Died:Once(function() current_char = nil end) end
    end

    if Player.Character and Player.Character.Parent then hookchar(Player.Character) end
...
wait(0.5)
getgenv().get_human = getgenv().get_human or function(Player)
    local char = g.Character or getgenv().get_char(Player)
    ...
    if h and h:IsDescendantOf(game) then
        h.Died:Connect(function() died = true end)
    end

    while not hum and not died do task.wait() end
    if hum_conn then hum_conn:Disconnect() end
    return (not died) and hum or nil
end

getgenv().get_root = getgenv().get_root or function(Player)
    local char = g.Character or getgenv().get_char(Player)
    if not char then return nil end
    ...
    local died = false
    local hum = char:FindFirstChildOfClass("Humanoid")
    if hum and hum:IsDescendantOf(game) then hum.Died:Connect(function() died = true end) end
    local added_conn
...
getgenv().get_head = getgenv().get_head or function(Player)
    local char = g.Character or getgenv().get_char(Player)
    if not char then return nil end
    ...
    while not head and not died do task.wait() end
    if added_conn then added_conn:Disconnect() end
...
local function find_game_data_folder_rs()
    local cache = g.Replic_Storage_Game_Data_HideAndSeek_Folder
    ...
    for _, v in ipairs(ReplicatedStorage:GetDescendants()) do
        if v:IsA("Folder") and v.Name:lower():find("game") and v.Name:lower():find("data") then
            g.Replic_Storage_Game_Data_HideAndSeek_Folder = v
            return v
        end
    end
    return nil
...
wait(0.1)
if not g.Replic_Storage_Game_Data_HideAndSeek_Folder then pcall(function() find_game_data_folder_rs() end) end

local function find_ability_handler_Module_Script()
    local cache = g.Ability_Handler_Module_Found
    if cache and cache:IsA("ModuleScript") then return cache end
    wait(0.1)
    for _, v in ipairs(g.PlayerScripts:GetDescendants()) do
        if v:IsA("ModuleScript") and v.Name:lower():find("ability") and v.Name:lower():find("handler") then
            g.Ability_Handler_Module_Found = v
            return v
        end
    end
    return nil
...
wait(0.1)
if not g.Ability_Handler_Module_Found then pcall(function() find_ability_handler_Module_Script() end) end

local function find_it_value(game_data)
    if getgenv().Game_Data_It_Value and getgenv().Game_Data_It_Value:IsDescendantOf(game_data) then return getgenv().Game_Data_It_Value end
...
local function create_label(target, text, color)
    if target:FindFirstChild("esp_label") then return target.esp_label end
...
local function clear_esp(char)
    ...
    if head and head.Parent and head:IsDescendantOf(game) then
...
local function apply_esp(plr, it_name)
    local char = plr.Character or g.get_char(plr, 1)
...
if not getgenv().player_ESP_Main_Task then
    getgenv().player_esp_loop = true
    getgenv().player_ESP_Main_Task = task.spawn(function()
        while getgenv().player_esp_loop == true do
            task.wait(0.25)
...
local function find_boombox()
    local root = getgenv().HumanoidRootPart or get_root(LocalPlayer, 5)
...
local function does_boombox_sound_exist()
    local root = getgenv().HumanoidRootPart or g.get_root(LocalPlayer, 5)
...
local function is_playing_music()
    local root_part = getgenv().HumanoidRootPart or g.get_root(LocalPlayer, 5)
...
local table_of_ids = {
    ...
    {id = 135011843473128, desc = "UNLEAKED"},
    {id = 140364174350628, desc = "PIMP DAT HOE!"},
    {id = 116601339086598, desc = "wombo combo"},
    {id = 88676102177987, desc = "VITAMINA"},
    ...
    {id = 119515456297024, desc = "Mexican Rap"},
...
local test = low_level_executor()
if test == false then
    Main:Toggle("No Cooldowns (FE)", g.Ability_Cooldown_Bypassed or false, function(active)
        local GameRunning = Game_Data:FindFirstChild("GameRunning")
        if not GameRunning then return g.notify("Error", "GameRunning flag was not found in Game_Data", 3) end
        if not InGame_LocalPlr_Value.Value then return g.notify("Error", "You're not currently in the game, this will work once you're in-game and you're IT.", 7.5) end
        g.Ability_Cooldown_Bypassed = active
        if active then
            local ItSeeking = Game_Data:FindFirstChild("ItSeeking")
            FlamesLibrary.spawn("ability_cooldown_bypass", "spawn", function()
                local AbilityHandler = require(g.Ability_Handler_Module_Found)
                while g.Ability_Cooldown_Bypassed == true do
                    if GameRunning.Value and ItSeeking and ItSeeking.Value then
                        pcall(function() AbilityHandler:SetCooldown(0) end)
                    else
                        repeat task.wait(0.1) until not g.Ability_Cooldown_Bypassed or (GameRunning.Value and ItSeeking and ItSeeking.Value)
                        if not g.Ability_Cooldown_Bypassed then break end
                        pcall(function() AbilityHandler:SetCooldown(0) end)
                    end
                    task.wait(0.1)
                end
            ...
            FlamesLibrary.disconnect("ability_cooldown_bypass")
        end
    end)
end
